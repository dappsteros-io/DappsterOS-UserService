...
readonly CASA_EXEC=dappsteros-user-service
readonly CASA_SERVICE=dappsteros-user-service.service
...
if [[ -z "${CASA_CONF}" ]]; then
    CASA_CONF=/etc/dappsteros/user-service.conf
fi

CASA_DB_PATH=$( (grep -i dbpath "${CASA_CONF}" || echo "/var/lib/dappsteros/db") | cut -d'=' -sf2 | xargs )
